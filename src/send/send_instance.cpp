...
void SendInstance::Send(const Napi::CallbackInfo &info) {
  if (info.Length() == 0 || info[0].IsUndefined() || info[0].IsNull() ||
    ...
    Napi::TypeError::New(info.Env(),
                         "The frames argument is expected to be an Array")
        .ThrowAsJavaScriptException();
    return;
  }
  ...
  for (uint32_t idx = 0; idx < frames.Length(); idx++) {
    ...
    if (frame.Has("audio") && !frame.Get("audio").IsUndefined() &&
        !frame.Get("audio").IsNull()) {
      const NDIlib_audio_frame_v3_t audioFrame =
          (AudioFrame)frame.Get("audio").As<Napi::Object>();
      NDIlib_send_send_audio_v3(this->ndiSendInstance, &audioFrame);
      free(audioFrame.p_data);
    }

    if (frame.Has("video") && !frame.Get("video").IsUndefined() &&
        !frame.Get("video").IsNull()) {
      try {
        const NDIlib_video_frame_v2_t ndiVideoFrame =
            (VideoFrame)frame.Get("video").As<Napi::Object>();
